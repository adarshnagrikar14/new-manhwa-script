...
class MultiSingleStreamBlockLoraProcessor(nn.Module):
    ...
    def __call__(self,
        ...
        for i in range(self.n_loras):
            query = query + self.lora_weights[i] * \
                self.q_loras[i](hidden_states)
            key = key + self.lora_weights[i] * self.k_loras[i](hidden_states)
            value = value + self.lora_weights[i] * \
                self.v_loras[i](hidden_states)
        ...
        mask = torch.ones((scaled_seq_len, scaled_seq_len),
    ...
    def __call__(self,
        ...
        for i in range(self.n_loras):
            hidden_states = hidden_states + \
                self.lora_weights[i] * self.proj_loras[i](hidden_states)
        # dropout
